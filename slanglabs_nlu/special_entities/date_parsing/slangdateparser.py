...
from slanglabs_nlu.special_entities.date_parsing.DateLexer import DateLexer
from slanglabs_nlu.special_entities.date_parsing.DateListener import DateListener  # noqa
from slanglabs_nlu.special_entities.date_parsing.DateParser import DateParser
...
NOW = datetime.now()
# NOW = datetime(year=2022, month=9, day=30)
...
class DateExtractionListener(DateListener):
    ...
    def exitLiteral_month(self, ctx: DateParser.Literal_monthContext):
        ...
        if day and is_valid(self.datetime.year, m, day):
...
def fmt(items):
    ret = []
    for item in items:
        ret.append((item[0].strftime('%Y-%m-%d'), item[1]))
    return ret


if __name__ == "__main__":
    val = 'tuesday of next week'
