...
from slanglabs_nlu.special_entities.date_parsing.slangdateparser import parse_dates  # noqa
...
test_cases = {
    ...
    'October 20': fmt((
        datetime(
            ...
            day=20,
            year=NOW.year+1
            if NOW.month > 12 or NOW.month == 12 and NOW.day >= 20 else NOW.year   # noqa
        ), (0, 9)
    )),
    ...
    'dhule to pune on 7 August': fmt((
        datetime(
            month=8,
            day=7,
            year=NOW.year if NOW.month < 8 or NOW.month == 8 and NOW.day < 7 else NOW.year + 1   # noqa
        ), (17, 24)
    )),
    ...
    'karana to travel from kota Rajasthan to lalitpur on 4th August': fmt((
        datetime(
            month=8,
            day=4,
            year=NOW.year if NOW.month < 8 or NOW.month == 8 and NOW.day < 4 else NOW.year + 1    # noqa
        ), (52, 61)
    )),
    'karana to travel from devri Rajasthan to Talbehat on 31st July': fmt((
        datetime(
            month=7,
            day=31,
            year=NOW.year if NOW.month < 7 or NOW.month == 7 and NOW.day < 32 else NOW.year + 1    # noqa
        ), (53, 61)
    )),
    ...
    '28th seventh month': fmt((
        datetime(
            month=7,
            day=28,
            year=NOW.year if NOW.month < 7 or NOW.month == 7 and NOW.day < 28 else NOW.year+1  # noqa
        ), (0, 17)
    )),
    'heroine on 30th': fmt((
        datetime(
            day=30,
            month=NOW.month if NOW.day < 30 and NOW.month != 2 else NOW.month + 1,    # noqa
            year=NOW.year
        ),
        (11, 14)
    )),
    ...
    'lucknow to kota on 11th August': fmt((
        datetime(
            day=11,
            month=8,
            year=NOW.year if NOW.month < 8 or NOW.month == 8 and NOW.day < 11 else NOW.year + 1   # noqa
        ),
        (19, 29)
    )),
    ...
    'delhi to haridwar on 30th July': fmt((
        datetime(
            day=30,
            month=7,
            year=NOW.year if NOW.month < 7 or NOW.month == 7 and NOW.day < 30 else NOW.year+1   # noqa
        ),
        (21, 29)
    )),
    'Parel mumbai': '',
    ...
    'Train options of 29 July to go to pandharpur': fmt((
        datetime(
            day=29,
            month=7,
            year=NOW.year if NOW.month < 7 or NOW.month == 7 and NOW.day < 29 else NOW.year+1   # noqa
        ), (17, 23)
    )),
    ...
    'delhi to lucknow is scheduled to go on 31st September': '',
    '3172022': fmt((
        datetime(
            day=31,
            month=7,
